 ...
 */
jQuery(function($) {
  var bucketUrl = "https://developer-uploaded-assets.s3.amazonaws.com",
  ...
  $("#uploader").fineUploaderS3({
    ...
    .on('complete', function(event, id, name, response, xhr) {
      ...
      var staffUploaderLinks = $fileEl.find(".staff-uploader-links")
      ...
      if (response.success) {
        staffUploaderLinks.show()
        staffUploaderLinks.find('a:first').attr('href', url)
        staffUploaderLinks.find('a:nth-child(2)').attr('href', getManifestFile())
      }

      function getManifestFile() {
        var manifestUrl = 'http://s3.amazonaws.com/sized-' + getExtensionCategory(key) + '-assets-public-v1/' + key

        return manifestUrl.substr(0, manifestUrl.lastIndexOf('.')) + '.html'

        function getExtensionCategory(fileName) {
          var fileExtension = getFileExtension(fileName)
          var recognizedExtensionCategories = {
            image: ['gif', 'jpeg', 'png'],
            video: ['avi', 'mpeg', 'mov', 'mp4', 'ogg', 'm4v', 'webm'],
          }

          return _.findKey(recognizedExtensionCategories, function(extensions) {
            return _.contains(extensions, fileExtension)
          }) || staffUploaderLinks.find('a:nth-child(2)').hide()
        }
